...
router.post("/intake", async (req: Request, res: Response) => {
  ...
  try {
    ...
    broadcast(clinicId, { type: "ER_INTAKE_CREATED", payload: { intakeId: row.id } });
...
router.patch("/intake/:id/assign", requireAssignableRole, async (req: Request, res: Response) => {
  ...
  try {
    ...
    broadcast(clinicId, { type: "ER_INTAKE_UPDATED", payload: { intakeId } });
...
router.post("/handoffs", requireAssignableRole, async (req: Request, res: Response) => {
  ...
  try {
    ...
    broadcast(clinicId, { type: "ER_HANDOFF_CREATED", payload: { handoffId: row.id, itemIds: row.itemIds } });
    res.status(201).json(row);
  } catch (err) {
    const code = err instanceof Error ? (err as Error & { code?: string }).code : undefined;
...
router.post("/handoffs/:id/ack", async (req: Request, res: Response) => {
  ...
  try {
    ...
    broadcast(clinicId, { type: "ER_HANDOFF_ACKNOWLEDGED", payload: { itemId } });
    res.status(200).json(row);
  } catch (err) {
    const code = err instanceof Error ? (err as Error & { code?: string }).code : undefined;

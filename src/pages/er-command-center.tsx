...
const ER_QUERY = ["er", "board"] as const;
const ASSIGNEES_QUERY = ["er", "assignees"] as const;
const ELIGIBLE_HOSP_QUERY = ["er", "eligible-hospitalizations"] as const;
...
export default function ErCommandCenterPage() {
  ...
  useEffect(() => {
    connectRealtime((ev) => {
      if (
        ev.type === "ER_INTAKE_CREATED" ||
        ev.type === "ER_INTAKE_UPDATED" ||
        ev.type === "ER_HANDOFF_CREATED" ||
        ev.type === "ER_HANDOFF_ACKNOWLEDGED" ||
        ev.type === "ER_HANDOFF_SLA_BREACHED"
      ) {
        invalidateEr();
      }
    });

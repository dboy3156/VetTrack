...
Authoritative implementation seams for the ER Wedge (ER Mode, **ER Allowlist**, **Concealment 404**, board **Primary Lane** / **Risk Badge** contracts):

- **[`shared/er-mode-access.ts`](shared/er-mode-access.ts)** — **ER Allowlist** path rules and SPA allowlist for **Concealment 404**; server and client import the same symbols for parity.
- **[`shared/er-types.ts`](shared/er-types.ts)** — Frozen public API terms and response shapes (e.g. **Queue Severity**, **Primary Lane**, handoff payloads) for the `/api/er` surface.

Server wiring: `server/middleware/er-mode-concealment.ts` (Concealment 404), `server/lib/er-mode.ts` (clinic **ER Mode** state), `server/middleware/tenant-context.ts` and `sessionContextMiddleware` in `server/middleware/auth.ts` (clinic context before policy). Client: `src/features/er/components/ErModeGuard.tsx` mirrors SPA **Concealment 404** for staff routes.

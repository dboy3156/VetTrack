...
The frontend always wraps the app in `<ClerkProvider>`. Without `VITE_CLERK_PUBLISHABLE_KEY`, the Clerk SDK may error in the browser. The **backend** has a dev-mode bypass (hardcoded admin user when no `CLERK_SECRET_KEY` is set), so API routes work without Clerk keys. To test the full UI end-to-end, Clerk test keys (`pk_test_*` / `sk_test_*`) are needed.
